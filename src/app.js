...
function _getEnvOriginal() {
    ...
    // FORCE OVERWRITE WITH LATEST MARKETING ASSETS
...
function renderContacts() {
    ...
    const bindRowActions = (element, c) => {
        ...
        const btnAff = element.querySelector(".btn-make-affiliate");
            ...
            tr.innerHTML = `
                ...
                <td style="text-align:right;">
                    <div class="kanban-card-actions" style="display:inline-flex; gap:2px;">
                        <button class="btn-icon-only btn-make-affiliate" title="Tornar este cliente um Afiliado" style="color:#06B6D4; width:26px; height:26px;"><i data-lucide="share-2" style="width:13px;height:13px;"></i></button>
...
function renderFinance() {
    ...
    const totalExpenses = (filteredExpenses||[]).reduce((s, e) => s + (e.value||0), 0) + totalDirectProductCosts;
...
let marketingDisplayMode = "table"; // 'table' or 'grid'
...
function renderMarketingAssets() {
    ...
    // 1. RENDER TABLE MATRIX VIEW (CONFORME O PDF EXECUTIVO DE ESTRATÉGIAS)
    filtered.forEach(asset => {
        ...
        tr.innerHTML = `
            <td style="padding: 14px; vertical-align: top;">
                ...
                <strong style="display:block; font-size:13.5px; color:var(--text-primary); font-weight:700;">${asset.channel || asset.title}</strong>
                ${asset.url ? `<a href="${asset.url}" target="_blank" style="font-size:11px; color:var(--color-primary); text-decoration:none; display:inline-flex; align-items:center; gap:2px; margin-top:4px;"><i data-lucide="external-link" style="width:10px; height:10px;"></i> ${asset.url.replace(/^https?:\/\//, '')}</a>` : ''}
            </td>
            <td style="padding: 14px; vertical-align: top;">
                <span style="font-weight:600; color:var(--text-primary); display:block;">${asset.approach || 'Tráfego & Prospecção'}</span>
                ${asset.responsible ? `<span style="font-size:11px; color:#06B6D4; font-weight:700; display:block; margin-top:4px; background:rgba(6,182,212,0.08); padding:2px 6px; border-radius:4px; width:fit-content;">👤 ${asset.responsible}</span>` : ''}
            </td>
            <td style="padding: 14px; vertical-align: top; line-height: 1.5;">
                <div style="background: var(--bg-app); border: 1px solid var(--border-color); padding: 10px 12px; border-radius: 6px;">
                    <span style="white-space: pre-line; color: var(--text-primary); font-size: 12px;">${asset.format || 'Definição de criativos e anúncios.'}</span>
                </div>
            </td>
            <td style="padding: 14px; vertical-align: top; line-height: 1.5;">
                <div style="background: rgba(5,150,105,0.06); border: 1px solid rgba(5,150,105,0.2); padding: 10px 12px; border-radius: 6px;">
                    <span style="white-space: pre-line; color: var(--text-primary); font-size: 12px; font-weight: 500;">${asset.strategy || 'Captura via WhatsApp/Formulário.'}</span>
                </div>
            </td>
...
function populateCustomerSelectForAffiliate(selectedContactId = null) {
